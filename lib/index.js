 ...
 * @returns {info}
 */
async function get_info(video_link,proxy) {
    let info = new Object();
    let link2info, title2info;
    
    try {
        link2info = await get_link2info(proxy?proxy+encodeURIComponent(video_link):video_link);
        title2info = await get_title2info(proxy,link2info.title);
